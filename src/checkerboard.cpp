...
public:
    ...
    // Normalize UV coordinates using scaling and offset
    Point2f normalizedUV = (uv.cwiseQuotient(m_scale) - m_delta);

    // Determine the coordinates of the checkerboard tile using modulo
    int checkerboardX = static_cast<int>(std::floor(normalizedUV.x())) % 2;
    int checkerboardY = static_cast<int>(std::floor(normalizedUV.y())) % 2;

    // Return m_value1 or m_value2 based on the checkerboard pattern logic
    if ((checkerboardX + checkerboardY) % 2 == 0) {
        return m_value1; // Return the first color
    } else {
        return m_value2; // Return the second color
    }
}
